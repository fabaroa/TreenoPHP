<?php
//$Id: updateFTsearch.php 15115 2015-01-23 20:25:14Z root $
...
foreach( $res as $row ){
	...
	foreach( $res2 as $row2 ){
		...
		//error_log(print_r($MaxCabKeyArr,true));
		...
		if ($rescount[0]['cnt']) fwrite($fp,"Folder Count for ".$row2['real_name'].":".$rescount[0]['cnt']."\n");
				...
				*/
				...
				if (count($existRecord)>0) {
					...
					if ($IsDeleted) {
						...
						$updateQry="update search set fulltext_content='".str_replace("'","''",$metadata)."',lastupdated='".date('Y-m-d H:i:s')."', cabinet='".$row2['departmentname']."' where type='folder' and cabinet_id=".$row2['departmentid']." and doc_id=".$folder['docid'];
					}
					//fwrite($fp,$updateQry."\n");
					...
					if (!$IsDeleted) {
						//insert
						$insertQry="insert into search (type,cabinet_id,doc_id,document_id,file_id,lastupdated,cabinet,fulltext_content) values ('folder','".$row2['departmentid']."','".$folder['docid']."',-1,-1,'".date('Y-m-d H:i:s')."','".$row2['departmentname']."','".str_replace("'","''",$metadata)."')";
		...
		$rescount=$DBDept->queryAll($queryA);
		echo($queryA."\nThe Count ".$rescount[0]['cnt']."\n");
		if ($rescount[0]['cnt']) fwrite($fp,"File Count for ".$row2['real_name'].":".$rescount[0]['cnt']."\n");
		$oldx=0;
		for ($x=0;$x<$rescount[0]['cnt'];$x+=100000) {
			$endX=$x+100000;
			...
			foreach ($files as $file){
				...
				if (count($existRecord)>0) {
					...
					if ($IsDeleted || IsDeleted($file['docid'],$file['id'],$row2['real_name'],$DBDept)) {
						...
						$updateQry="update search set cabinet='".$row2['departmentname']."',fulltext_content='".str_replace("'","''",$metadata)."',info='".$file['parent_filename']."',lastupdated='".$dateCreated."' where type='file' and cabinet_id=".$row2['departmentid']." and doc_id=".$file['docid']." and file_id=".$file['id'];
					}
					//fwrite($fp,$updateQry."\n");
					$statis=$DBSearch->queryAll($updateQry);
				} else {
					if (!$IsDeleted && !IsDeleted($file['docid'],$file['id'],$row2['real_name'],$DBDept)) {
						//insert
						$insertQry="insert into search (type,cabinet_id,doc_id,document_id,file_id,lastupdated,cabinet,info,fulltext_content) values ('file','".$row2['departmentid']."','".$file['docid']."',-1,'".$file['id']."','".$dateCreated."','".$row2['departmentname']."','".$file['parent_filename']."','".str_replace("'","''",$metadata)."')";
						$statis=$DBSearch->query($insertQry);
						//fwrite($fp,$insertQry."\n");
					}
				}
		...
		//cz 2015-01-13
		$queryDelDocuments = "SELECT * FROM ".$row2['real_name']."_files WHERE filename IS NULL AND deleted=1 AND TimeStamp>".$fileKey." AND TimeStamp<=".$MaxFileKey;
		$delDocuments = $DBDept->queryAll($queryDelDocuments);
		//echo "Delected documents: ".$queryDelDocuments."\n".count($delDocuments)."\n";
		foreach ($delDocuments as $delDocument){			
			$deleteQry="delete from search where  type='docType' and cabinet_id=".$row2['departmentid']." and doc_id=".$delDocument['doc_id']." and file_id=".$delDocument['id']." and document_id=".$delDocument['document_id'];
			//echo "deleteQry: ".$deleteQry."\n";
			$statis=$DBSearch->queryAll($deleteQry);
			
			$queryFilesInDelDocument = "SELECT id FROM ".$row2['real_name']."_files WHERE filename IS NOT NULL AND subfolder='".$delDocument['subfolder']."' AND doc_id=".$delDocument['doc_id'];
			$filesInDelDocument = $DBDept->queryAll($queryFilesInDelDocument);
			//echo "files In DelDocument: ".$queryFilesInDelDocument."\n".count($filesInDelDocument)."\n";
			foreach ($filesInDelDocument as $fileInDelDocument){	
				$deleteFileQry="delete from search where type='file' and cabinet_id=".$row2['departmentid']." and doc_id=".$delDocument['doc_id']." and file_id=".$fileInDelDocument['id']." and document_id=-1";
				//echo "deleteFileQry: ".$deleteFileQry."\n";
				$statis=$DBSearch->queryAll($deleteFileQry);
			}
			
			$deleteDocTypeValues ="DELETE FROM docTypeValues WHERE document_table_name='".$delDocument['document_table_name']."' AND document_id = '".$delDocument['document_id']."'";
			//echo "deleteDocTypeValues: ".$deleteDocTypeValues."\n";
			$statis=$DBSearch->queryAll($deleteDocTypeValues);
		}	
		...
		$rescount=$DBDept->queryAll($queryA);
		if ($rescount[0]['cnt']) fwrite($fp,"DocType Count for ".$row2['real_name'].":".$rescount[0]['cnt']."\n");
		$oldx=0;
		for ($x=0;$x<$rescount[0]['cnt'];$x+=100000) {
			$endX=$x+100000;
			...
			foreach ($files as $file){
				...
				$metadataQry="select [id],[document_defs_list_id],[document_id],[document_field_defs_list_id],CONVERT(TEXT, document_field_value) AS document_field_value from document_field_value_list where id=".$file['id'];
				$docTypeIndex=$DBDept->queryAll($metadataQry);
				$metadataQry= "select [id],[document_defs_list_id],[document_id],[document_field_defs_list_id],CONVERT(TEXT, document_field_value) AS document_field_value from document_field_value_list where document_id=".$docTypeIndex[0]['document_id']." and document_defs_list_id=".$docTypeIndex[0]['document_defs_list_id'];
				$metadataArr=$DBDept->queryAll($metadataQry);
				//echo $metadataQry."\n";
				//print_r($metadataArr);
				$metadata="";
				...
				//fwrite($fp,$metadataQry."\n");
				$metadataArr=$DBDept->queryAll($metadataQry);
				dbErrLocal ($metadataArr,$fp);
				//fwrite($fp,print_r($metadataArr,true)."\n");
				if (count($metadataArr)>0) {
					...
					if (count($existRecord)>0) {
						...
						if ($IsDeleted || IsDeleted($metadataArr[0]['doc_id'],$metadataArr[0]['file_id'],$row2['real_name'],$DBDept)) {
							...
							$updateQry="update search set info='".$docTypeNameArr[0]['document_type_name']."',cabinet='".$row2['departmentname']."',fulltext_content='"."Created by ".$metadataArr[0]['created_by']."||".str_replace("'","''",$metadata)."',lastupdated='".$metadataArr[0]['date_modified']."' where  type='docType' and cabinet_id=".$row2['departmentid']." and doc_id=".$metadataArr[0]['doc_id']." and file_id=".$metadataArr[0]['file_id']." and document_id=".$metadataArr[0]['id'];
						}
						//fwrite($fp,$updateQry."\n");
						$statis=$DBSearch->queryAll($updateQry);
					} else {
...
function IsDeleted($doc_id,$file_id,$cab,$DBDept) {
	...
	if (count($resultArr)>0) {
		...
		if ($file_id > 0) {
			...
			if ($IsDeleted) return $IsDeleted;
			//echo "is file version?".$IsDeleted."\n".$query."\n";
			if (isset($resultArr1[0]) && $resultArr1[0]['display']==0) $IsDeleted = 1;
			if ($IsDeleted) return $IsDeleted;
			//check document type
			...
			if ($subfolder) {
				...
				if (isset($resultArr2[0]) && $resultArr2[0]['document_table_name'] && $resultArr2[0]['document_id']) {
					$document_table_name=$resultArr2[0]['document_table_name'];
					$document_id = $resultArr2[0]['document_id'];
					$query="select * from ".$document_table_name." where id=".$document_id;
					//echo $query;
					//print_r($DBDept);
...
function dbErrLocal(&$res,$fp,$kill=1) {
	global $DEFS;
	if(PEAR::isError($res)) {
		$bt = $res->backtrace;
