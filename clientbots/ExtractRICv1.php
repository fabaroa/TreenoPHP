...
foreach($cabinets as $cabinetRes){
	...
	if ($cabinet=="DocStar")
	{
		$doc_idQ="select doc_id from ".$cabinet." where deleted=0 and doc_id>119934";
	}
	else
	{
		$doc_idQ="select doc_id from ".$cabinet." where deleted=0";
	}
	...
	foreach ($doc_ids as $doc_idTmp)	{
		...
		foreach ($folders as $folder) {
			...
			$queryB="select * from ".$cabinet."_files where display=1 and deleted=0 and doc_id=".$folder['doc_id']." order by id,subfolder".$limit;
			//echo $queryB."\n";
			$fileNames=$db_dept->queryAll($queryB);
			$folderMetaData="";
			foreach($cabinetIndices as $indexValue){
				if ($indexValue != "doc_id" && $indexValue != 'location' && $indexValue != 'deleted' && $indexValue != 'timestamp') $folderMetaData=$folderMetaData.$folder[$indexValue]."\t";
			}
			//$nonFileChar = array(" ", "'", "$", "&", "<", ">", ":", '"', "/","\\","|","?","*",".","(",")");
			...
			if( !is_dir( $home."/".$department."/".$cabinet."/".$folder['doc_id']))
			//create directory with 777 perms
			mkdir( $home."/".$department."/".$cabinet."/".$folder['doc_id'], 0777 );
			$fp2=fopen($home."/".$department."/".$cabinet."/".$folder['doc_id']."/INDEX.txt","w");
			fwrite($fp2,$folderMetaData);
			fclose($fp2);
			
			foreach($fileNames as $fileName){
				if( !is_dir( $home."/".$department."/".$cabinet."/".$folder['doc_id']."/".$fileName['subfolder'] ) )
				//create directory with 777 perms
				mkdir( $home."/".$department."/".$cabinet."/".$folder['doc_id']."/".$fileName['subfolder'], 0777 );
				$message="";
				if ($fileName['filename'] == NULL){
					...
					if ($fileName['subfolder']) {
						$dest=$home."/".$department."/".$cabinet."/".$folder['doc_id']."/".$fileName['subfolder']."/";
					} else {
						$dest=$home."/".$department."/".$cabinet."/".$folder['doc_id']."/";
					}
					...
					if (!copy($source.$fileName['filename'],$dest.$fileName['filename'])) {
						$message = "problem:".$source.$fileName['filename']." ".$dest.$fileName['filename']."\n";
						$message += $temp."\t".$fileName['filename']."\t".$fileName['date_created']."\n";
					} else {
	...
	//$cmd="zip -r ".$zipToPath.$cabinet.".zip ".$home."/".$department."/".$cabinet;
	//echo $cmd."\n";
