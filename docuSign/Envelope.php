...
{    
	...
	protected function _docusignClientCallMethod($methodName, array $params, $debug = true)
	{
       ...
       $tmpSoapFault = new SoapFault('mycode', 'mystring', 'myactor', 'mydetail', 'myname', 'myheader');
	//   xdebug_disable();
       try
       {      	
       		//if(count($params) == 1)
       		//{
       		//	$response = $this->_docusignClient->__soapCall($methodName,  $params);
       		//}
       		//else if(count($params) > 1)
       		//{
       			$response = $this->_docusignClient->__soapCall($methodName, array('parameters' => $params));  
       		//}
       		//else
       		//{
       		////	error_log("params error occured in _docusignClientCallMethod():  ".$methodName);
       		//}     
       		unset($tmpSoapFault);
       }
       catch(SoapFault $ex)
       {
       		$tmpSoapFault = new SoapFault($ex->faultcode, "A SoapFault error occured when invoking DocuSign API (\"$methodName\"): ".$ex->getMessage(), 
       										$methodName, 'mydetail', 'myname', 'myheader');
       		
  			$err = $ex->getMessage();
			$errFile = $ex->getFile();
			//$errCode = $ex->getCode();
			$errLine = $ex->getLine();
			error_log("SoapFault exception occured in _docusignClientCallMethod($methodName):  ".$err);
       }
       //xdebug_enable();
       
       if(isset($tmpSoapFault))
       {
       		throw $tmpSoapFault;
       }
       //testing - throw a simulated DocuSign API error
       //throw new SoapFault($methodName, 'simulated DocuSign API error', 'myactor', 'mydetail', 'myname', 'myheader');
