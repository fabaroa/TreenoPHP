...
if(isset($_SESSION['user'])) {
	...
	$webuser = getNextUser("client_files7");
	$userName = $webuser['username'];
	$password = $webuser['password'];
}
$data = "<doc><username>$userName</username>";
$data .= "<password>$password</password>";
$data .= "<department>client_files7</department>";
if(!empty($autosearch)) {
	$data .= "<term index=\"subject\" value=\"$autosearch\" />";
	$data .= "<cabinet>City_Manager_Correspondence_to_City_Council</cabinet>";
}else{
	$data .= "<term index=\"subject\" value=\"$autosearch\" />";
	$data .= "<cabinet>City_Manager_Correspondence_to_City_Council</cabinet>";
}
