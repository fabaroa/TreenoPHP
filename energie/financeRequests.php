...
$department = "client_files3";
$autosearch = $_GET['autosearch'];
$cabinet = "Bids_and_Requests_for_Proposal";
$index = "fiscal_year";

$db_dept = getDbObject($department);
$testCab = getTableInfo($db_dept, "departments", array('departmentid'), array("real_name" => $cabinet), 'queryOne');
if($testCab == NULL) {
	die("Please contact your server administrator 1");
}

$indices = getCabinetInfo($db_dept, $cabinet);
if( !in_array($index, $indices) ) {
	die("Please contact your server administrator 2");
}
...
if(isset($_SESSION['user'])) {
	...
	$webuser = getNextUser($department);
...
if(!empty($autosearch)) {
	$data .= "<term index=\"$index\" value=\"$autosearch\" />";
	$data .= "<cabinet>$cabinet</cabinet>";
}else{
	$data .= "<term index=\"$index\" value=\"$autosearch\" />";
	$data .= "<cabinet>$cabinet</cabinet>";
}
