<?php
//$Id: indexing2.php 15029 2013-08-27 14:40:52Z fabaroa $
...
$db_name, $existing, $settObj, $DEFS ){
	//cz 2015-11-09
	//error_Log("copyFiles() from_path: ".$from_path.", to_path: ".$to_path);
//error_log("copyFiles() existing=".$existing);
	...
	if( sizeof( $fl ) > 0 )
	{
		for( $j=0; $j < sizeof($fl); $j++ )
		{
			...
			//error_Log("copyFiles() documentName: ".$documentName);//cz 2015-11-09
			//$t_dir = strtolower("$to_path/{$fl[$j]}"); //since this is not being produce during migration
			$t_dir = "$to_path/{$fl[$j]}";
			if( !is_dir( $t_dir ) ) {
			...
			if( !is_dir( $t_dir ) ) {
				...
				$documentTableName=$getDocTypeRes[0]['document_table_name']; //need to get the document
				$cabQuery="select departmentid from departments where real_name='".$cab."'";
				$getCabIDRes = $db_dept->queryAll($cabQuery);
				$db_dept->disconnect();
				$cabinetID = $getCabIDRes[0]['departmentid'];
				$indices = array();
				$subfolderID = createDocumentInfoHERE($db_name,$cabinetID,$doc_id,$documentTableName,$indices,$username, $db_doc, $db_object);
				//*******End of NEW CODE
				...
				$message .= "Cabinet: $cab, Folder: ".$foldername."Tab Name: ".$fl[$j];
	...
	for( $j=0; $j<sizeof( $fl ); $j++ ){
		...
		unset( $fileSize );
		unset( $fileCreated );
		//$subfolder	= strtolower($fl[$j]);	//cz 2015-11-09: this causes problem - inconsistent with how existingFiles is built and how C#-side uploading files
		$subfolder	= $fl[$j];
		$docuSubfolder=$subfolder;
		if( !is_dir( $to_path."/".$docuSubfolder ) ) {
		...
		copyAndSQL(	$filesList,$orderingOffset,$docuSubfolder,$sqlarr,"$to_path/$docuSubfolder","$from_path/$subfolder",$fileSize,$fileCreated,$doc_id,$username, $db_name, $existingFiles[$docuSubfolder],$centeraModule);
	}
	unlockTables($db_object);
	$table = $cab."_files";
	for( $i=0; $i<sizeof($sqlarr);$i++ ){
