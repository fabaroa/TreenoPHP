<?php
// $Id: webServices.php 14992 2013-05-08 19:41:21Z cz $
...
function getUserWorkflowTodoList ($db_doc, $userName, &$arbList, &$allArbCabs,
	...
	foreach ($todoList as $todoArr) {
		...
		if($wfDep != $curWFDep) {
			if (isset($clientDB)) $clientDB->disconnect();
...
function copyDocuments($username, $curInfo, $destInfo, $db_doc, $db_dept, $dest_db_dept) {
	...
	if($cab !== false) {
		...
		if($destCab !== false) {
			...
			error_log($cur_location);
			error_log($dest_location);
...
function createCabinetFolder($dept, $cabinetID, $indiceArr, $userName, $db_doc, $db_dept) {
	...
	if($row = $cabInfo->fetchRow() and in_array($row['real_name'], $cabArr)) {
		$temp_table = '';
		$docID = (int) createFolderInCabinet($db_dept, $gblStt, $db_doc, $userName, $dept, $row['real_name'], array_values($indiceArr), array_keys($indiceArr), $temp_table);
	} else {
		$docID = -1;
	}
	return $docID;
...
{
//error_log("getAutoComplete() $department");
 	global $DEFS;
	...
	elseif( strcmp($autoComplete, "odbc_auto_complete") == 0 )
	{
		...
		//cz
		if(($department =='client_files300')||
		($department =='client_files132' && (strtolower($cabinetName)=='clients' || strtolower($cabinetName)=='policies' || strtolower($cabinetName)=='claims' )))
		{
			$indices = getCabinetInfo($db_dept, $cabinetName);
			$row = getTableInfo($db_dept, $cabinetName, array(), array($indices[0] => $autoCompleteTerm), 'queryRow');
			$db_dept->disconnect ();			
			if(count($row) > 0)	{	
				//error_log("getAutoComplete() This record already exists - ".print_r($row, true));
				 foreach($indices AS $arrKey => $cabIndex){
						$autoCompleteValues[$cabIndex] = $row[$cabIndex];
				}
				return $autoCompleteValues;
			}
			else{
				//error_log("getAutoComplete() This record not exist yet");
...
function moveMainSubfolderFiles($db_dept, $cabinet, $docID, $destCabinet, $destDocID, $mainFilesArr, $copy=true)
{
	...
	foreach($mainFilesArr AS $filesRow) {
		...
		if($copy) {
			if(copy($cur_location."/".$filename, $dest_location."/".$filename)) {
				moveFilesInDB($db_dept, $cabinet, $destCabinet, $destDocID, array($filesRow), NULL, $copy);
			} else {
				return "$filename failed to be copied from the Main tab";
			}
		} else {
			if(rename($cur_location."/".$filename, $dest_location."/".$filename)) {
				moveFilesInDB($db_dept, $cabinet, $destCabinet, $destDocID, array($filesRow), NULL, $copy);
			} else {
				return "$filename failed to be moved from the Main tab";
			}
		}
	}
	return true;
