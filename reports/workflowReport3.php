...
if($logged_in == 1 && strcmp($user->username,"") != 0) {
	$db_debt = $user->getDbObject();

	$completeUserList = getTableInfo($db_debt,'access',array('username'),array(),'queryCol');
...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en">
	...
	function genReport() {
		...
		var user = getEl('userInput').value;
		if(date1 != "") {
			window.location = "wf_report3.php?start_date=" + date1+ "&end_date=" + date2 + "&user=" + user;
		} 
	}
	</script>
</head>
<body class="centered" onload="registerFuncs()">
	<div>
		<label for="userInput">User to Report on:<label>
		<select style='margin:0;padding:0' id='userInput' name='completeUserList'>
		<?php foreach($completeUserList AS $uname): ?>
